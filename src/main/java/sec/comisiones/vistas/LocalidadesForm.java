...
import sec.comisiones.manager.LocalidadesManager;
...
public class LocalidadesForm extends CustomComponent {

	private VerticalLayout mainLayout;
	private Panel contenedorPanel;
	private VerticalLayout verticalLayout_1;
	final LocalidadesManager tblUser = new LocalidadesManager();
	private Table tb;
	NativeButton btnNew=new NativeButton();
	Usuarios usrLog = new Usuarios();
	private ProvinciasDAO sa = new ProvinciasDAO();
	
	public LocalidadesForm() {
		ini();
	}
	
	public LocalidadesForm(Usuarios usr) {
		usrLog= usr;
		ini();
	}
	...
	private void ini(){
		...
		btnNew.addClickListener(new ClickListener(){
			public void buttonClick(ClickEvent event) {
				...
				LocalidadesDAO ud=new LocalidadesDAO();
				Item item = ud.getItem();
							
	//		    Notification.show("item selected: "+ item.getItemProperty("idProvincia").getValue(), Notification.Type.HUMANIZED_MESSAGE ); 
...
///////////////////////////////////////////////	
	...
	class MyForm extends FormLayout {
	    ...
	    TextField kmDesdeOrigen = new TextField("kms");
	    ComboBox idProvincia = new ComboBox("Provincia");
	    ...
	    HorizontalLayout hl_0 = new HorizontalLayout(nombre, kmDesdeOrigen, idProvincia);
	    ...
	    public void setItemDataSource(Item item) {
	    	idProvincia.setContainerDataSource(sa.getAllProvinciasContainer());
	...
	//        binder.bindMemberFields(this);
	    ...
	    //	btnModifica.setDescription("Guarda Modificaciones");
	    	...
	    	kmDesdeOrigen.addStyleName("Small");
	    	kmDesdeOrigen.setRequired(true);
	    	kmDesdeOrigen.setRequiredError("Campo Obligatorio");
	    	kmDesdeOrigen.addValidator(new FloatRangeValidator("Rango Permitido [0,0 - 3.000,0]",0.0f,3000.0f));    
	    	kmDesdeOrigen.setImmediate(true);
	    	kmDesdeOrigen.setValidationVisible(true);
	    	kmDesdeOrigen.setWidth("10.0%");

	    	
	    	
	    	idProvincia.addStyleName("Small");
	    	idProvincia.setRequired(true);
	    	idProvincia.setRequiredError("Campo Obligatorio");
	    	idProvincia.setItemCaptionPropertyId("nombre");
	    	idProvincia.setImmediate(true);
	    	idProvincia.setInvalidAllowed(true);
	    	idProvincia.setNullSelectionAllowed(true);
	    	idProvincia.setTextInputAllowed(false);
	    	...
	    	if ((Integer) item.getItemProperty("id").getValue() == 0){
		    	...
		    	nombre.setValue("");
		    	kmDesdeOrigen.setValue("0.0");
		    	idProvincia.setValue("");
				tb.setSelectable(false);
	    	}
	    	...
	    	btnGraba.addClickListener(new ClickListener(){
				public void buttonClick(ClickEvent event) {
						try {
							...
							LocalidadesDAO u = new LocalidadesDAO();
							float num=new Formateos().convertirComaPorPunto(kmDesdeOrigen.getValue()).floatValue();
							if(btnGraba.getCaption()=="Graba Nuevo"){
								try {
									 if ((int)idProvincia.getValue()>0){
										 
										 if (u.existe(nombre.getValue())==0){
											u.agrega(nombre.getValue(), num, (int)idProvincia.getValue(), usrLog.getId()); 
											...
											ini();
											 ...
											 Notification.show("Debe seleccionar una Provincia. ", Type.ERROR_MESSAGE);
										 }
									} catch (SQLException e) {
										e.printStackTrace();
									}
								} else if (btnGraba.getCaption()=="Actualiza"){
									u.modifica(nombre.getValue(), num, (int)idProvincia.getValue(), usrLog.getId()); 		
									ini();
								}
