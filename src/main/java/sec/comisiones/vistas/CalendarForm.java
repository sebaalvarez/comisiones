...
public class CalendarForm extends GridLayout implements View {
	...
	// lo utilizo desde la ventana de comisiones menu para ver todas o las generadas por usuario
	public CalendarForm(String claseEvento, String param1, String param2) {
		this.tipoEvento=claseEvento;
		this.param1=param1;
		this.param2=param2;
	...
	// lo utilizo desde la ventana de comisiones para ver disponibilidades
	public CalendarForm(String claseEvento, Date fechaInicio) {
		this.tipoEvento=claseEvento;
	...
	public CalendarForm(String claseEvento) {
		this.tipoEvento=claseEvento;
		// handleURLParams(request.getParameterMap());
		// testBench = ValoThemeUI.isTestMode();
	...
	public void initContent() {
		setSizeFull();
		setHeight(UtilUI.devuelveAltoCuerpo(),Unit.PIXELS);
		// setMargin(true);
		setSpacing(true);
	...
	private void cargarEventos() {
		...
		BeanItemContainer<BasicEvent> container1 = new BeanItemContainer<BasicEvent>(BasicEvent.class);
		EventosDAO container = new EventosDAO();
		String fecIni="";
		String fecFin="";
		boolean venc=false;
		container1 = container.getAllEventosContainer(tipoEvento,param1,param2,fecIni,fecFin,venc);
 

		calendarComponent.setContainerDataSource(container1, "caption","description", "start", "end", "styleName");

		/*
		 * // We can also add events to the container through the calendar
		 * BasicEvent event = new
		 * BasicEvent("Wednesday Wonder","Wonderful Event",new
		 * GregorianCalendar(2012,1,15,15,00).getTime(),new
		 * GregorianCalendar(2012,1,15,19,00).getTime());
		 * event.setStyleName("Red"); 
		 * calendarComponent.addEvent(event);
		 */

		// The container MUST be ordered by the start time. You have to sort the
		// BIC every time after you have added items.
		container1.sort(new Object[] { "start" }, new boolean[] { true });

	}

	
	@SuppressWarnings("serial")
	class detalleEventoPopup extends Window {
	    public detalleEventoPopup(String caption, Consumer<String> save) {
	    		super("Detalle");
	    		center();
	        setModal(true);
	//        setClosable(false);
	        ...
	        //Llama a la pantalla de consulta del evento
//	        CalendarDetalleForm vusuarios7 = new CalendarDetalleForm(usrLog,5);

	        // el tama�o del caption esta relacionado con lo que se le cargue en la consulta
	        //Notification.show("","'"+caption.substring(6, 12)+"'",Type.ERROR_MESSAGE);
	        
	        
	        
	        if(caption.substring(0, 4).equals("Com.")){   
	        		this.setWidth(120, Unit.EM);
	        		ComisionesForm vusuarios6=new ComisionesForm(usrLog,caption.substring(6, 12),"cns");
	        		content.addComponents(vusuarios6);
	        	
	        }else if(caption.substring(0, 4).equals("Res.")) {
	        		this.setCaption("Reserva Libre");
	        		this.setWidth(60, Unit.EM);
//		        this.setHeight(70, Unit.PERCENTAGE);
		        ReservasForm vusuarios6=new ReservasForm(usrLog,caption.substring(6, 12),"cns");
		        content.addComponents(vusuarios6);
	...
	/*
	 * 
	 * private void addInitialEvents() { Date originalDate = calendar.getTime();
	 * Date today = getToday();
	 * 
	 * // Add a event that last a whole week
	 * 
	 * Date start = resolveFirstDateOfWeek(today, calendar); Date end =
	 * resolveLastDateOfWeek(today, calendar); CalendarTestEvent event =
	 * getNewEvent("Whole week event", start, end); event.setAllDay(true);
	 * event.setStyleName("color4");
	 * event.setDescription("Description for the whole week event.");
	 * dataSource.addEvent(event);
	 * 
	 * // Add a allday event calendar.setTime(start);
	 * calendar.add(GregorianCalendar.DATE, 3); start = calendar.getTime(); end
	 * = start; event = getNewEvent("All-day event", start, end);
	 * event.setAllDay(true); event.setDescription("Some description.");
	 * event.setStyleName("color3"); dataSource.addEvent(event);
	 * 
	 * // Add a second allday event calendar.add(GregorianCalendar.DATE, 1);
	 * start = calendar.getTime(); end = start; event =
	 * getNewEvent("Second all-day event", start, end); event.setAllDay(true);
	 * event.setDescription("Some description."); event.setStyleName("color2");
	 * dataSource.addEvent(event);
	 * 
	 * calendar.add(GregorianCalendar.DATE, -3);
	 * calendar.set(GregorianCalendar.HOUR_OF_DAY, 9);
	 * calendar.set(GregorianCalendar.MINUTE, 30); start = calendar.getTime();
	 * calendar.add(GregorianCalendar.HOUR_OF_DAY, 5);
	 * calendar.set(GregorianCalendar.MINUTE, 0); end = calendar.getTime();
	 * event = getNewEvent("Appointment", start, end); event.setWhere("Office");
	 ...
	 * calendar.set(GregorianCalendar.MINUTE, 0); start = calendar.getTime();
	 * calendar.add(GregorianCalendar.HOUR_OF_DAY, 8); end = calendar.getTime();
	 ...
	 * calendar.set(GregorianCalendar.HOUR_OF_DAY, 9);
	 * calendar.set(GregorianCalendar.MINUTE, 0); start = calendar.getTime();
	 * calendar.add(GregorianCalendar.HOUR_OF_DAY, 9); end = calendar.getTime();
	 * event = getNewEvent("Free time", start, end); dataSource.addEvent(event);
	 * 
...
//		initAddNewEventButton();
		...
		hl_0.addComponent(group);
		hl_0.setComponentAlignment(group, Alignment.MIDDLE_CENTER);
		GridLayout g1 = new GridLayout(3, 1);
		g1.setWidth("100.0%");
		g1.addComponent(prevButton, 0, 0, 0, 0);
		g1.addComponent(captionLabel, 1, 0, 1, 0);
		g1.addComponent(nextButton, 2, 0, 2, 0);
		g1.setComponentAlignment(prevButton, Alignment.MIDDLE_LEFT);
		g1.setComponentAlignment(captionLabel, Alignment.MIDDLE_CENTER);
		g1.setComponentAlignment(nextButton, Alignment.MIDDLE_RIGHT);
		g1.setColumnExpandRatio(0, 1f);
		g1.setColumnExpandRatio(1, 1f);
		g1.setColumnExpandRatio(2, 1f);
		...
		// hl.setExpandRatio(captionLabel, 1.0f);
		...
		// controlPanel.setComponentAlignment(formatSelect,
		// Alignment.MIDDLE_LEFT);
		// controlPanel.setComponentAlignment(localeSelect,
		// Alignment.MIDDLE_LEFT);
		...
		// controlPanel.setComponentAlignment(addNewEvent,
...
//			testDate.set(GregorianCalendar.YEAR, 2017);
//			testDate.set(GregorianCalendar.MONTH, 1);
...
//		});
		...
		//		Notification.show("Obtengo Evento"+event.getCalendarEvent().getCaption().substring(8, 3),Type.ERROR_MESSAGE);
				String caption=event.getCalendarEvent().getCaption();
	...
	//			Notification.show("",caption.substring(6, 12),Type.ERROR_MESSAGE);
				...
				UI.getCurrent().addWindow(new detalleEventoPopup(caption,(newValue) -> { }));
			}
		});
...
//	private void detalleEventoPopup() {
...
//		scheduleEventPopup.setWidth(85, Unit.PERCENTAGE);
//		scheduleEventPopup.setHeight(85, Unit.PERCENTAGE);
//		scheduleEventPopup.setModal(true);
//		scheduleEventPopup.setResizable(true);
//		scheduleEventPopup.center();
//
//		
//		Usuarios usrLog=new Usuarios();
//		ConsultaPlanificacionForm vusuarios7 = new ConsultaPlanificacionForm(usrLog,127,3);
//		scheduleEventPopup.addComponents(vusuarios7);
//    	
//    	
//
//
//	}
	...
	private void createCalendarEventPopup1() {
		VerticalLayout layout = new VerticalLayout();
		// layout.setMargin(true);
		layout.setSpacing(true);
	...
	private void updateCaptionLabel() {
		...
		captionLabel.setWidth(15.0f, Unit.EM);
		captionLabel.setStyleName("negritaCentrado");
	}
